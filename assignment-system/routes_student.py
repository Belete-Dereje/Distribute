    ...
    if request.method == 'POST':
        ...
        if assignment and assignment['is_group']:
            ...
            if datetime.now() > deadline and has_late_permission:
                ...
                status = 'submitted'
            # create a group token id (UUID instead of timestamp)
            group_token = str(uuid.uuid4())
            now_ts = datetime.now()
            for sid in teammates:
                submission_uuid = str(uuid.uuid4())
                cur.execute("INSERT INTO submissions (id, assignment_id, student_id, files, student_comment, status, submitted_at, group_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
                            (submission_uuid, assignment_id, sid, files_str, comment, status, now_ts, group_token))
        ...
        if existing:
            ...
            cur.execute("""
                INSERT INTO submissions (id, assignment_id, student_id, files, student_comment, status)
                VALUES (?, ?, ?, ?, ?, ?)
            """, (str(uuid.uuid4()), assignment_id, student_id, files_str, comment, status))
...
@student_bp.route('/complain/<submission_id>', methods=['POST'])
@login_required
