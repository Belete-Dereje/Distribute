...
def register():
    if request.method == 'POST':
        ...
        try:
            ...
            cur.execute(
                "INSERT INTO users (id, user_id, first_name, last_name, email, password_hash, role, is_approved) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
                (user_uuid, user_id, first_name, last_name, email, password_hash, role, is_approved)
            )
            conn.commit()
            ...
            if role == 'student':
                ...
                if not department or not year:
                    ...
                    conn.close()
                    return render_template('register.html')
                # Generate UUID for student record
                student_uuid = str(uuid.uuid4())
                cur.execute("INSERT INTO students (id, user_id, department, year) VALUES (?, ?, ?, ?)", (student_uuid, new_user_id, department, int(year)))
            elif role == 'teacher':
                ...
                if not departments or not years or not courses:
                    ...
                    conn.close()
                    return render_template('register.html')
                # Generate UUID for teacher record
                teacher_uuid = str(uuid.uuid4())
                cur.execute("INSERT INTO teachers (id, user_id, departments, years, courses) VALUES (?, ?, ?, ?, ?)", (teacher_uuid, new_user_id, departments, years, courses))
