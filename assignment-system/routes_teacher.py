...
def create_assignment():
    ...
    if request.method == 'POST':
        ...
        cur.execute("""
            INSERT INTO assignments (id, title, description, teacher_id, course_name, department, year, deadline, late_submission, penalty_per_day, max_score, is_group, max_group_size, teacher_comment, files)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, (assignment_uuid, title, description, teacher_id, course_name, department, int(year), deadline_date, late_submission, penalty_per_day, max_score, is_group, max_group_size, teacher_comment, files))
...
@teacher_bp.route('/edit-assignment/<assignment_id>', methods=['GET', 'POST'])
@login_required
...
@teacher_bp.route('/submissions/<assignment_id>')
@login_required
...
@teacher_bp.route('/manage-late/<assignment_id>', methods=['POST'])
@login_required
...
@teacher_bp.route('/stats/<assignment_id>')
@login_required
...
@teacher_bp.route('/evaluate/<submission_id>', methods=['GET', 'POST'])
@login_required
...
@teacher_bp.route('/export-pdf/<assignment_id>')
@login_required
